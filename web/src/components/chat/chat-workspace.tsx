  ...
  return <main className="workspace">
    ...
    <section className="chat">
      ...
      {section === 'chat' ? <>
        ...
        <div className="operations-toolbar"><div><b>{detail ? 'عرض التفاصيل' : sectionTitle(section)}</b><small>{detail ? 'إجراءات وقراءة تشغيلية من بيانات IBEX الفعلية' : 'واجهة تشغيل مباشرة للبيانات الفعلية في IBEX'}</small></div>{!detail && section !== 'reports' && <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="بحث سريع…" />}</div>
